...
carol 서버로 POST하고 서버가 파싱해 저장한다. SEGA 로그인 비밀번호는 요청하거나 전송하지 않는다.
...
1. 디스코드에서 `/북마클릿` 실행 → "설치 가이드 열기" 링크를 받는다.
   링크의 호스트가 이전 도메인이어도 토큰만 추출하므로 사용할 수 있다.
...
- 웹스토어 리스팅에 [carol 개인정보처리방침](https://maimai.team-carol.com/privacy) 연결
