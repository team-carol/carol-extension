...
`pnpm dev`는 크롬을 자동으로 띄우고 코드 수정을 즉시 반영합니다.
다만 **개발 서버가 켜져 있는 동안에만 동작합니다** — dev 빌드는 매니페스트에
`content_scripts`가 없고 서버가 런타임에 스크립트를 주입하는 구조라,
서버를 끄면 확장이 설치돼 있어도 페이지에서 아무 일도 일어나지 않습니다.

서버 없이 그냥 쓰려면 프로덕션 빌드를 로드하세요.
...
> **`.output`이 안 보인다면** — 점으로 시작하는 폴더라 macOS 파일 선택창에서 기본으로 숨겨집니다.
> `Cmd + Shift + .`로 숨김 파일을 표시하거나, `Cmd + Shift + G`로 경로를 직접 붙여넣으세요.
> 프로젝트 루트에는 `manifest.json`이 없으므로 루트를 고르면 "매니페스트 파일이 없다"는 오류가 납니다.

dev 빌드와 프로덕션 빌드는 `.output/chrome-mv3` 같은 자리를 덮어씁니다.
`pnpm dev`를 쓰다가 서버 없이 쓰려면 `pnpm build`를 다시 실행하세요.
